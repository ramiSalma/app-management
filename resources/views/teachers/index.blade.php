...
<div class="container mx-auto px-4">
    ...
    <a href="{{url('/teachers/create')}}" class="bg-cyan-500  p-4 text-white rounded-xl shadow-lg hover:shadow-cyan-500/100">add teacher</a>
            ...
            <div class="flex flex-col items-center pb-6">
                ...
                <div class="flex mt-4">
                    <form action="{{url('/teachers/'. $teacher->id)}}" method="post">
                        @csrf
                        @method('DELETE')
                        <a onclick="return confirm('are you sure?')"  class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-cyan-600 rounded-lg hover:bg-cyan-700">delete </a>
                    </form>
                    <a href="{{url('/teachers/'.$teacher->id.'/edit')}}" class="py-2 px-4 ms-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg hover:bg-gray-100 hover:text-blue-700">Update</a>
                </div>
            </div>
        </div>
        @endforeach
